...
> **쇼핑쇼츠 자동 제작을 찾아오셨다면** → [`CLAUDE.md`](CLAUDE.md) 와 [`prompts/`](prompts/) 부터 보세요.
> 영상을 `inbox/` 에 넣고 클로드 코드에서 `/쇼츠` 한 번이면 상품분석 → 키워드 → 경쟁분석 →
> 대본 → 제목 → 캡션·썸네일 → 댓글이 순서대로 돌고, 그 결과가 아래 reelforge 로 넘어갑니다.
> 각 단계가 *무엇을 생각할지* 는 `prompts/*.md` 에 직접 써넣습니다.
